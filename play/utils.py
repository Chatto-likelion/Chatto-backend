...
#         [당신의 임무]
#         요청자가 제시한 관계에 편향되지 말고, 주어진 대화 내용만을 근거로 두 사람의 '썸' 성공 가능성과 현재 관계의 분위기를 **매우 객관적으로 분석**해야 합니다.
#         반드시 지정된 출력 형식에 맞춰 결과를 작성해주세요. 다른 부가 설명은 절대 추가하지 마세요.
...
#         6.  **대화 주도권 분석**: A와 B가 새로운 대화 주제를 시작한 비율을 합계 100%가 되도록 추정하고, 주제를 시작하는 스타일을 한 줄로 설명한 뒤, 실제 예시를 각각 들어주세요. (예시가 없으면 '없음'). 
#         7.  **평균 메시지 길이 분석**: A와 B의 평균 메시지 길이를 각각 계산해주세요. (단위: 글자수) 마지막으로 대화 패턴을 2문장으로 요약해주세요.
#         8.  **종합 상담**: '챗토'의 입장에서, 두 사람의 관계를 긍정적으로 요약하고 응원하는 3~4문장의 따뜻한 상담 메시지와, 관계 발전을 위한 1~2문장의 실용적인 팁을 작성해주세요. 마지막으로 전체 분석에 사용된 메시지 수를 작성해주세요.
...
#             "len_A": parse_response(r"A 평균 메시지 길이:\s*(\d+)", response_text, is_int=True),
#             "len_B": parse_response(r"B 평균 메시지 길이:\s*(\d+)", response_text, is_int=True),
#             "len_A_desc": parse_response(r"A 메시지 특징:\s*(.+)", response_text),
#             "len_B_desc": parse_response(r"B 메시지 특징:\s*(.+)", response_text),
#             "len_A_ex": parse_response(r"A 메시지 예시:\s*(.+)", response_text),
#             "len_B_ex": parse_response(r"B 메시지 예시:\s*(.+)", response_text),
...
#             "num_chat": parse_response(r"분석에 사용된 메시지 수:\s*(\d+)", response_text, is_int=True),
#         }
#         return results
